...
void Textos::setup(){
    ...
    if(ofGetWidth() > 768){
        myfont.loadFont("of_data/aleixo/darkheart-regular.ttf", 35*2);
        //myfont.setLineHeight(150.0f);
        //myfont.setLetterSpacing(1);
    } else{
        myfont.loadFont("of_data/aleixo/darkheart-regular.ttf", 35);
    }
...
void Textos::draw(int screen_){
    ...
    for (int i = 0; i< nunberOfLines; i++){
        ...
        //goHorse para acertar o espassamento
        
        int espassamento = 20;
        int rectH;
        
        if(ofGetWidth() > 768){
            rectH = rect.height + espassamento*2;
        }else{
            rectH = rect.height + espassamento;
        }
        
        float x = (ofGetWidth()/2) - rect.width/2;
        float y = ((ofGetHeight()-(rectH*(nunberOfLines)))/2)+(i+1)*rectH;
